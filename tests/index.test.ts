...
import { APIPromise } from 'orb-billing/core/api-promise';

import util from 'node:util';
import Orb from 'orb-billing';
import { APIUserAbortError } from 'orb-billing';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    const forceAPIResponseForClient = async (client: Orb) => {
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['ORB_LOG'] = 'debug';
      const client = new Orb({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['ORB_LOG'] = 'not a log level';
      const client = new Orb({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'ORB_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Orb({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Orb({ apiKey: 'My API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['ORB_API_KEY'] = 'My API Key';
    const client = new Orb();
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Orb({ apiKey: 'My API Key', fetch: testFetch });
